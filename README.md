...
## Хід виконання (крок за кроком по TRM)

Покроковий журнал «який розділ TRM читав → що він означає → що зробив у коді»:
...
Полярність звірено з TRM ESP32-S3 v1.8 (дослівний опис поля з `soc/rmt_struct.h`,
згенерованого з мануала):
...
Отже `= 0` → доступ через FIFO (`CHnDATA`), `= 1` → прямий доступ до RMT-RAM.
У тексті ДЗ це місце сформульовано інвертовано; тут виправлено за мануалом.
